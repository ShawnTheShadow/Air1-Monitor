...
fn test_app_initialization_with_explicit_config() {
    ...
    assert!(app.status.is_empty(), "unexpected status on init: {}", app.status);
    assert_eq!(app.cfg.mqtt.keepalive_secs, 42);

    // cleanup and restore env
    let _ = fs::remove_dir_all(&base);
    if let Some(prev) = prev {
        unsafe { env::set_var("XDG_CONFIG_HOME", prev); }
    } else {
...
fn test_app_default_status_is_empty() {
    ...
    assert!(app.status.is_empty(), "default app had non-empty status: {}", app.status);
}